const configPromise = require('@root/config');
const actionPromise = require('@lib/updateActions');
const { getRemoteRev } = require('@root/lib/svn/getSVNRev');
...
class Observer {
  ...
  async main() {
    ...
    this._intervalHandle = setInterval(() => {
      if (
        !config.repos.filter((r) => r.errorCount <= this.maxErrorCount).length
      ) {
        console.log(
